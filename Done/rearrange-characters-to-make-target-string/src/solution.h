/*
https://leetcode.com/problems/rearrange-characters-to-make-target-std::string/
...
26
Companies
You are given two 0-indexed strings s and target. You can take some letters from
s and rearrange them to form new strings.
...
into "code". We can make at most two copies of "code", so we return 2. Example
2:
...
class Solution {
   public:
    int rearrangeCharacters(std::string s, std::string target) {
        ...
        for (auto it : tCnt) {
            debug("tCnt", it.first, it.second);
        }

        for (auto it : sCnt) {
            debug("sCnt", it.first, it.second);
        }

        int canFit = s.size();

        for (auto it = tCnt.begin(); it != tCnt.end(); ++it) {
            if (sCnt.count(it->first)) {
                int temp = sCnt[it->first] / it->second;
                debug("", temp, it->first);
                if (temp < canFit) canFit = temp;
            } else {
                return 0;
            }
        }
