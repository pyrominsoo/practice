...
void traverse(map<int,int>& rightview, queue< pair<Node*,int> >& node_queue);
void rightView(Node *root)
{
	map<int,int> rightview;
	queue< pair<Node*, int> > node_queue;
	if (root) {
		// Traverse the tree, breadth-first, carry the vertical distance to root	
		node_queue.push(make_pair(root,0));
		traverse(rightview, node_queue);	
        for (map<int,int>::const_iterator it = rightview.begin(); it != rightview.end(); it++) {
            cout << it->second << " ";
        }
	} else {
		return;
...
void traverse(map<int,int>& rightview, queue< pair<Node*,int> >& node_queue) {
	while (!node_queue.empty()) {
        ...
        node_queue.pop();
        rightview[curr_vd] = curr_node->data;
        if (curr_node->left) {
